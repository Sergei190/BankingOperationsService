...
import ru.effectivemobile.boperations.boundary.AppProfileEmailCrudInteractor;
import ru.effectivemobile.boperations.boundary.request.AppProfileEmailRequest;
...
@AllArgsConstructor
@RestController
@RequestMapping(value = "/profiles/{userId}/emails", produces = MediaType.APPLICATION_JSON_VALUE)
@UserPermission
public class AppProfileEmailController {

    private final AppProfileEmailCrudInteractor interactor;
    ...
    @PostMapping
    public AppProfilePropertyDto<String> create(@Valid @RequestBody AppProfileEmailRequest request,
    ...
    public AppProfilePropertyDto<String> update(@PathVariable UUID propertyId,
            @Valid @RequestBody AppProfileEmailRequest request,
            @AuthenticationPrincipal AppUserDetails userDetails) {
    ...
    @Operation(summary = "Delete email", description = "Last email cannot be deleted")
    @Parameter(name = "userId", in = ParameterIn.PATH, schema = @Schema(format = "uuid"))
    @Parameter(name = "propertyId", in = ParameterIn.PATH, schema = @Schema(format = "uuid"))
